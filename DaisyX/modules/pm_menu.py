...
async def get_start_func(message, strings, edit=False):
    ...
    buttons.add(
        InlineKeyboardButton(strings["btn_channel"], url="https://t.me/SoFiaUpdates_Official"),
        InlineKeyboardButton("👥 Support Grup", url="https://t.me/SoFiaUpdates_Official"),
    )
    buttons.add(
        InlineKeyboardButton(
            "👸🏼 Add SoFiaX to your group",
            url=f"https://telegram.me/TheSoFiaXBot?startgroup=true",
        )
    )
...
async def help_cmd_g(message, strings):
    ...
    button = InlineKeyboardMarkup().add(
        InlineKeyboardButton(text=text, url="https://t.me/TheSoFiaXBot?start")
    )
    await message.reply(strings["help_header"], reply_markup=button)
